...
for i in range(len(df) - 37):
    ...
    x = df.iloc[i+37, 2]
    y_pred.append(a*x + b)
    # print(a*x+b, df.iloc[i+37, 1])
y_true = df.iloc[37:, 1]
err = sklearn.metrics.mean_squared_error(y_true, y_pred)
print(err)
plt.figure()
plt.plot(df.iloc[37:, 0], df.iloc[37:, 2] * a0 + b0, label="static")
plt.plot(df.iloc[37:, 0], y_true, label="true")
plt.plot(df.iloc[37:, 0], y_pred, label="dynamic")
plt.legend()
plt.show()
